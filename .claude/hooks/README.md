...
단 `public_guard.self` 는 훅 폴더 안이어도 검사한다 — 거기 적은 낱말은 그대로 공개되기 때문이다.
...
**자기 이름은 `public_guard.self` 에 적는다** (`<저장소>/.claude/hooks/`, 한 줄 하나, `#` 주석).
여기 적힌 낱말은 합친 목록에서 뺀다 — 부모 목록에 제 툴 이름이 있어도 제 문서가 걸리지 않는다.
공개해도 되는 값이라 `.words` 와 달리 git 에 넣는다.

`.self` 에는 **그 저장소 이름만** 적을 수 있다. 검사기는 적힌 낱말을 저장소 이름
(폴더 이름 · `git remote get-url origin` 의 끝 이름)과 맞춰 보고, **다르면 빼 주지 않고 경고만 찍는다.**
아무 낱말이나 적어 감지를 끄지 못하게 하려는 것이다.

`.self` 는 **검사 대상 파일이 있는 폴더에서 부모로 거슬러 올라가며** 찾는다 (`.words` 와 같은 방식).
그래서 부모 저장소의 훅이 `Tools/<툴>/` 서브모듈 문서를 검사할 때도 그 서브모듈의 `.self` 가 먹는다.
